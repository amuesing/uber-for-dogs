...
I am making full use of AR Macros to accomplish CRUD by using the instance methods has_many, and belongs_to. User has_many appointments, and also has_many dog-walker associations via the Appointment Class. Like-wise, dog-walker has_many appointments, and also has_many user associations via the Appointment Class.

The CLI is completely linear in flow. The instance of the User is assisgned the variable "id", and the variable is passed to all subsequent methods via an argument.

--Welcome--
This a the default welcome message that greets a user when they open the app.

--Login or Signup--
The user is prompted to indicated whether they would like to "login", or "sign up". Depending on the user choice, the user will be directed to either the sign_up method, or the login method. Notably, this is the last method that is specifically called on in our runner method.

--Sign Up--
The user is prompted to provide a name, and username, and from this a new user instance is created and assigned the variable "id".

--Login--
The user is prompted to "Enter Username". If the username is present in the datebase, login method will return "Welcome Back!", and assign the user instance to the variable "id". If the username is not present in the database, the login method will return "Username Unknown" and direct the user to the sign_up method.

--Create Appointment--
Create appointment allows the user to input the username of a dog-walker, and a date, and instantiates a new instance of the Appointment class, and because it is a .create command, saves to database. The create_appointment method dove tails into see_all_appointments via the variable that the user instance has been assigned to "id".

--See All Appointments--
The users is prompted to answer whether or not they would like to view all appointments associated with their user instance. If 'y' is selected, see_all_appointment method returns all of the Appointment Object_Id numbers associated with each Appointment instance. This is an issue which need resolution, since ideally here the Appointment would output the date of the appointments in question, and not simply the Object_Id associated with the user instance. The see_all_appointments method dove tails into cancel_appointment method via the variable that the user instance has been assigned to "id".

--Cancel Appointments--
After the user is propted to indicate whether or not they would like to cancel an appointment. If the user chooses 'y' the user will be asked to indicate the desired date of the cancelled appointment. Once cancelled, cancel_appointment method will ask whether or not the user would like to schedule another appointment, at which point they will be directed to the create_appointment method. 
